...
Private Sub cmdAceptar_Click()
    ...
    Else
        ...
        sql = sql & " ,CCL_FECPC=" & XDQ(FechaProx.Value)
...
Private Sub cmdAceptarPedido_Click()
    ...
    If txtnroPedido.Text = "" Then
        ...
        sql = sql & " ESP_CODIGO,CLI_CODIGO,PED_MOTIVO,PED_DESCRI)"
        sql = sql & " VALUES ("
        sql = sql & Num & ","
...
Private Function validarPedido() As Boolean
    ...
    If txtDescPedido.Text = "" Then
        MsgBox "No ha ingresado el pedido", vbCritical, TIT_MSGBOX
        txtPedido.SetFocus
        validarPedido = False
        Exit Function
    End If
...
Private Function CargarPedidosAnteriores()
    ...
    sql = "SELECT P.*,V.VEN_NOMBRE,C.CLI_RAZSOC"
    sql = sql & " FROM PEDIDO P, CLIENTE C, VENDEDOR V "
    sql = sql & " WHERE P.CLI_CODIGO = C.CLI_NRODOC"
    sql = sql & " AND P.VEN_CODIGO = V.VEN_CODIGO"
    If txtBuscaCliente.Text <> "" Then
        sql = sql & " AND P.CLI_CODIGO = " & XN(txtBuscaCliente)
    End If
...
'llamada aplicacion que de pacientes
    ...
    If cboDocCon.ListIndex <> -1 Then
        ...
        sql = sql & " AND TUR_FECHA = " & XDQ(Fecha.Value)
        sql = sql & " ORDER BY TUR_HORAD ASC "
...
Private Function preparogrillas()
    ...
    grdConsultas.FormatString = "Fecha|Doctor|Motivo|Indicaciones|FechaProx|CodMedico|CCL_NUMERO"
    ...
    ' Grilla de pedidos
    grdPedidos.FormatString = "Fecha|Descripcion|Motivo|PED_NUMERO|Cod cliente|VEN_CODIGO|Especialidad"
    ...
    grdPedidos.ColWidth(6) = 0 'especialidad
    grdPedidos.Rows = 1
...
Private Function CargarConsultasAnteriores()
    ...
    sql = sql & " WHERE CC.CLI_CODIGO = C.CLI_NRODOC"
    sql = sql & " AND CC.VEN_CODIGO = V.VEN_CODIGO"
    If txtBuscaCliente.Text <> "" Then
        sql = sql & " AND CC.CLI_CODIGO = " & XN(txtBuscaCliente)
    End If
    ...
    If Rec1.EOF = False Then
        Do While Rec1.EOF = False
            grdConsultas.AddItem Rec1!CCL_FECHA & Chr(9) & Rec1!VEN_NOMBRE & Chr(9) & Rec1!CCL_MOTIVO & Chr(9) & _
                                 Rec1!CCL_INDICA & Chr(9) & Rec1!CCL_FECPC & Chr(9) & Rec1!VEN_CODIGO & Chr(9) & Rec1!CCL_NUMERO
