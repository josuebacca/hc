...
Begin VB.Form ABMTipoProveedor 
   ...
   EndProperty
   Icon            =   "ABMTipoProveedor.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   210
      Picture         =   "ABMTipoProveedor.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   1605
      Width           =   330
...
Const cTabla = "TIPO_PROVEEDOR"
Const cCampoID = "TPR_CODIGO"
Const cDesRegistro = "Tipo de Proveedor"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Tipo de Proveedor..."
            txtID_LostFocus
            DesacCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Tipo de Proveedor..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos del Tipo de Proveedor..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Tipo de Proveedor..."
            DesacCtrl txtID
    End Select
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (TPR_CODIGO, TPR_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     TPR_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE TPR_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE TPR_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE TPR_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!TPR_CODIGO
                txtDescri.Text = rec!TPR_DESCRI
            Else
                Beep
