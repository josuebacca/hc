VERSION 5.00
Begin VB.Form ABMAparato 
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Datos de Aparatos..."
   ClientHeight    =   2025
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
...
Const cTabla = "APARATO"
Const cCampoID = "APT_CODIGO"
Const cDesRegistro = "Aparato"
...
Function SetMode(pMode As Integer)
    ...
    Select Case pMode
        Case 1
            ...
            Me.Caption = "Nuevo Aparato..."
            txtID_LostFocus
            DesacCtrl txtID
        Case 2
            cmdAceptar.Enabled = False
            Me.Caption = "Editando Aparato..."
            DesacCtrl txtID
        Case 3
            cmdAceptar.Visible = False
            Me.Caption = "Datos del Aparato..."
            DesacCtrl txtID
        Case 4
            cmdAceptar.Enabled = True
            Me.Caption = "Eliminando Aparato..."
            DesacCtrl txtID
    End Select
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (APT_CODIGO, APT_DESCRI) "
                cSQL = cSQL & "VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "     APT_DESCRI = " & XS(txtDescri.Text)
                cSQL = cSQL & " WHERE APT_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE APT_CODIGO  = " & XN(txtID.Text)
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE APT_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                'si encontró el registro muestro los datos
                txtID.Text = rec!APT_CODIGO
                txtDescri.Text = rec!APT_DESCRI
            Else
                Beep
