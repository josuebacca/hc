VERSION 5.00
Begin VB.Form ABMLabClinicos 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Datos del Laboratorio Clinico...."
   ClientHeight    =   4410
   ClientLeft      =   2700
   ClientTop       =   2625
   ClientWidth     =   4530
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   495
      Left            =   3960
      Picture         =   "ABMLabClinicos.frx":000C
      ScaleHeight     =   495
      ScaleWidth      =   495
      TabIndex        =   22
      Top             =   120
   ...
   Begin VB.ComboBox cboLocalidad 
      Height          =   315
      ItemData        =   "ABMLabClinicos.frx":0A0E
      Left            =   1065
      List            =   "ABMLabClinicos.frx":0A10
   ...
   Begin VB.ComboBox cboProvincia 
      Height          =   315
      ItemData        =   "ABMLabClinicos.frx":0A12
      Left            =   1065
      List            =   "ABMLabClinicos.frx":0A14
      Style           =   2  'Dropdown List
      TabIndex        =   3
      Top             =   1425
      Width           =   3375
   ...
   Begin VB.ComboBox cboPais 
      Height          =   315
      ItemData        =   "ABMLabClinicos.frx":0A16
      Left            =   1065
      List            =   "ABMLabClinicos.frx":0A18
      Style           =   2  'Dropdown List
      TabIndex        =   2
      Top             =   1080
      Width           =   3375
   ...
   Begin VB.CommandButton cmdAyuda 
      Height          =   315
      Left            =   240
      Picture         =   "ABMLabClinicos.frx":0A1A
      Style           =   1  'Graphical
      TabIndex        =   14
      Top             =   3960
      Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "ABMLabClinicos"
...
Const cTabla = "LAB_CLINICOS"
Const cCampoID = "LAC_CODIGO"
Const cDesRegistro = "Laboratorio Clinico"
    ...
    If Validar(vMode) = True Then
        ...
        Select Case vMode
            Case 1 'nuevo
                ...
                cSQL = cSQL & "     (LAC_CODIGO, LAC_NOMBRE, LAC_DOMICI, LAC_TELEFONO,"
                cSQL = cSQL & " LAC_MAIL, LAC_FAX, LOC_CODIGO,PRO_CODIGO,PAI_CODIGO,LAC_ESTADO) "
                cSQL = cSQL & " VALUES "
            ...
            Case 2 'editar
                ...
                cSQL = cSQL & "  LAC_NOMBRE=" & XS(txtNombre.Text)
                cSQL = cSQL & " ,LAC_DOMICI=" & XS(txtDomicilio.Text)
                cSQL = cSQL & " ,LAC_TELEFONO=" & XS(txtTelefono.Text)
                cSQL = cSQL & " ,LAC_MAIL=" & XS(txtMail.Text)
                cSQL = cSQL & " ,LAC_FAX=" & XS(txtFax.Text)
                ...
                If chkVenEstado.Value = Checked Then
                    cSQL = cSQL & " ,LAC_ESTADO = 'S'"
                Else
                    cSQL = cSQL & " ,LAC_ESTADO = 'N'"
                End If
                cSQL = cSQL & " WHERE LAC_CODIGO  = " & XN(txtID.Text)
            
            Case 4 'eliminar
            
                cSQL = "DELETE FROM " & cTabla & " WHERE LAC_CODIGO  = " & XN(txtID.Text)
        End Select
...
Private Sub Form_Load()
    ...
    If vMode <> 1 Then
        If vFieldID <> "0" Then
            cSQL = "SELECT * FROM " & cTabla & "  WHERE LAC_CODIGO = " & Mid(vFieldID, 2, Len(vFieldID) - 2)
            rec.Open cSQL, DBConn, adOpenStatic, adLockOptimistic
            If (rec.BOF And rec.EOF) = 0 Then
                txtID.Text = rec!LAC_CODIGO
                txtNombre.Text = rec!LAC_NOMBRE
                ...
                txtDomicilio.Text = ChkNull(rec!LAC_DOMICI)
                txtTelefono.Text = ChkNull(rec!LAC_TELEFONO)
                txtFax.Text = ChkNull(rec!LAC_FAX)
                txtMail.Text = ChkNull(rec!LAC_MAIL)
                
                If ChkNull(rec!LAC_ESTADO) = "N" Or ChkNull(rec!LAC_ESTADO) = "" Then
