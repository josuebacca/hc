...
Begin VB.Form frmNotaDeditoClienteConcepto 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Nota de Dédito Clientes por Concepto..."
   ClientHeight    =   7650
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11280
   ...
   Begin TabDlg.SSTab tabDatos 
      ...
      EndProperty
      TabCaption(0)   =   "&Datos"
      TabPicture(0)   =   "frmNotaDeditoClienteConcepto.frx":0000
      ...
      TabCaption(1)   =   "&Buscar"
      TabPicture(1)   =   "frmNotaDeditoClienteConcepto.frx":001C
      ...
      Begin VB.Frame FramePara 
         ...
         Begin VB.CommandButton cmdBuscarCliente 
            ...
            Left            =   1935
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaDeditoClienteConcepto.frx":0038
            Style           =   1  'Graphical
            TabIndex        =   66
      ...
      Begin VB.Frame frameBuscar 
         ...
         Begin VB.CommandButton cmdBuscarVen 
            ...
            Left            =   3570
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaDeditoClienteConcepto.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   65
         ...
         Begin VB.CommandButton cmdBuscarCli 
            ...
            Left            =   3570
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaDeditoClienteConcepto.frx":064C
            Style           =   1  'Graphical
            TabIndex        =   41
            ToolTipText     =   "Buscar Cliente"
            Top             =   255
         ...
         Begin VB.CommandButton cmdBuscarSuc 
            ...
            Left            =   3570
            MaskColor       =   &H000000FF&
            Picture         =   "frmNotaDeditoClienteConcepto.frx":0956
            Style           =   1  'Graphical
            TabIndex        =   40
   ...
   End
End
Attribute VB_Name = "frmNotaDeditoClienteConcepto"
...
Private Sub CmdBuscAprox_Click()
    ...
    sql = sql & " AND ND.NDC_SERVICHEQUE='O'" 'PARA QUE BUSQUE CONCEPTO
...
Private Sub CmdGrabar_Click()
    ...
    If rec.EOF = True Then
        ...
        sql = sql & XN(VIva) & ","
        sql = sql & "'O'" & "," 'SE TRATA DE CONCEPTO
        ...
        For i = 1 To grdGrilla.Rows - 1
            If grdGrilla.TextMatrix(i, 0) <> "" Then
                ...
                sql = sql & " REP_CODIGO, DND_NROITEM, DND_CONCEPTO,"
                sql = sql & " DND_PRECIO)"
                sql = sql & " VALUES ("
                sql = sql & XN(cboNotaDebito.ItemData(cboNotaDebito.ListIndex)) & ","
                sql = sql & XN(txtNroNotaDebito) & ","
                ...
                sql = sql & XDQ(FechaNotaDebito) & ","
                sql = sql & XN(cboRep.ItemData(cboRep.ListIndex)) & ","
                sql = sql & XN(grdGrilla.TextMatrix(i, 2)) & "," 'NRO DE ITEM
                sql = sql & XS(grdGrilla.TextMatrix(i, 0)) & "," 'CONCEPTO
                sql = sql & XN(grdGrilla.TextMatrix(i, 1)) & ")" 'IMPORTE
                DBConn.Execute sql
            End If
        Next
...
Private Sub CmdSalir_Click()
    If MsgBox("Seguro que desea Salir", vbQuestion + vbYesNo, TIT_MSGBOX) = vbYes Then
        Set frmNotaDeditoClienteConcepto = Nothing
        Unload Me
    End If
End Sub
...
Private Sub GrdModulos_dblClick()
    If GrdModulos.Rows > 1 Then
        ...
        sql = "SELECT DND_CONCEPTO, DND_PRECIO, DND_NROITEM"
        sql = sql & " FROM DETALLE_NOTA_DEBITO_CLIENTE"
        sql = sql & " WHERE NDC_NUMERO=" & XN(Right(GrdModulos.TextMatrix(GrdModulos.RowSel, 1), 8))
        sql = sql & " AND NDC_SUCURSAL=" & XN(Left(GrdModulos.TextMatrix(GrdModulos.RowSel, 1), 4))
        sql = sql & " AND REP_CODIGO=" & XN(GrdModulos.TextMatrix(GrdModulos.RowSel, 11))
        sql = sql & " AND NDC_FECHA=" & XDQ(GrdModulos.TextMatrix(GrdModulos.RowSel, 2))
        sql = sql & " AND TCO_CODIGO=" & XN(GrdModulos.TextMatrix(GrdModulos.RowSel, 8))
        sql = sql & " ORDER BY DND_NROITEM"
        Rec1.Open sql, DBConn, adOpenStatic, adLockOptimistic
        If Rec1.EOF = False Then
            i = 1
            Do While Rec1.EOF = False
                grdGrilla.TextMatrix(i, 0) = Rec1!DND_CONCEPTO
                grdGrilla.TextMatrix(i, 1) = Valido_Importe(Rec1!DND_PRECIO)
                grdGrilla.TextMatrix(i, 2) = Rec1!DND_NROITEM
                i = i + 1
                Rec1.MoveNext
            Loop
        End If
