...
Private Sub cmdAgregar_Click()
    ...
    If MsgBox("驴Confirma el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
    ...
    'For i = 1 To nFilaH - nFilaD
        ...
        Else
            ...
            If MsgBox("Ya hay un turno para ese horario 驴Confirma la Modificaci贸n del Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then
                rec.Close
                Exit Sub
            End If
            ' aca hago el update
    ...
    If MsgBox("驴Imprime el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then
        LimpiarTurno
        Exit Sub
    End If
    ImprimirTurno
...
Private Sub cmdQuitar_Click()
    ...
    If txtCodigo.Text <> "" Then
        If grdGrilla.TextMatrix(grdGrilla.RowSel, 1) <> "" Then
            If MsgBox("驴Confirma Eiminar el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
        ...
        Do While rec.EOF = False
            ...
            If Not (IsNull(rec!CLI_CUMPLE)) Then
                If rec.EOF = False Then
                    ...
                    If Month(Fecha) < Month(rec!CLI_CUMPLE) Then a駉s = a駉s - 1 'todav铆a no ha llegado el mes de su cumple
                    If Month(Now) = Month(rec!CLI_CUMPLE) And Day(Fecha) < Day(rec!CLI_CUMPLE) Then a駉s = a駉s - 1 'es el mes pero no ha llegado el d铆a de su cumple
                    edad = a駉s
                End If
            Else
                edad = 0
