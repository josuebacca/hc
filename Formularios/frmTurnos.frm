...
Private Sub cmdAgregar_Click()
    ...
    Dim a駉s, edad As Integer
    Dim Fecha As Date
    'Validar los campos requeridos
    If ValidarTurno = False Then Exit Sub
    'If ValidarHorarioTurno = False Then Exit Sub
    If MsgBox("緾onfirma el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
    ...
    'For i = 1 To nFilaH - nFilaD
        ...
        If Not rec.EOF = False Then
            ...
            'If User <> 99 Then
                sql = sql & " TUR_USER, "
            'End If
            sql = sql & " TUR_FECALTA, TUR_DESDE, TUR_IMPORTE, TUR_ORDEN, TUR_IMPRESO)"
            sql = sql & " VALUES ("
        ...
        Else
            ...
            If MsgBox("Ya hay un turno para ese horario 緾onfirma la Modificaci髇 del Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then
                rec.Close
                Exit Sub
            End If
            ' aca hago el update
        ...
        If Not (IsNull(rec!CLI_CUMPLE)) Then
                If rec.EOF = False Then
                    a駉s = Year(Date) - Year(rec!CLI_CUMPLE)
                    If Month(Fecha) < Month(rec!CLI_CUMPLE) Then a駉s = a駉s - 1 'todavia no ha llegado el mes de su cumple
                    If Month(Now) = Month(rec!CLI_CUMPLE) And Day(Fecha) < Day(rec!CLI_CUMPLE) Then a駉s = a駉s - 1 'es el mes pero no ha llegado el dia de su cumple
                    edad = a駉s
                End If
            Else
                edad = 0
        ...
        If MsgBox("縄mprime el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then
'
...
'    " 驴Confirma Copiar los Turnos?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
'
'    sAction = "COPIAR"
...
Private Sub cmdCortar_Click()
    ...
    " 驴Confirma Cortar los Turnos?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
...
'            If MsgBox("Hay Turnos previamente cargados en este dia que se eliminaran si realiza esta acci贸n." & Chr(13) & _
'            " 驴Confirma eliminar estos Turnos?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
'
...
'        " " & Chr(13) & "驴Confirma Pegar los Turnos?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
'
'        sql = "UPDATE TURNOS SET"
'        sql = sql & " TUR_FECHA = " & XDQ(MViewFecha.Value)
...
Private Sub cmdQuitar_Click()
    ...
    If txtCodigo.Text <> "" Then
        If grdGrilla.TextMatrix(grdGrilla.RowSel, 1) <> "" Then
            If MsgBox("緾onfirma Eiminar el Turno?", vbQuestion + vbYesNo, TIT_MSGBOX) = vbNo Then Exit Sub
    ...
    If rec.EOF = False Then
        ...
        Do While rec.EOF = False
            ...
            grdGrilla.AddItem Format(rec!TUR_HORAD, "hh:mm") & " a " & Format(rec!TUR_HORAH, "hh:mm") & Chr(9) & rec!CLI_RAZSOC & Chr(9) & edad & Chr(9) & ChkNull(rec!CLI_TELEFONO) & Chr(9) & ChkNull(rec!CLI_CELULAR) & Chr(9) & rec!TUR_OSOCIAL & Chr(9) & ChkNull(rec!TUR_MOTIVO) & Chr(9) & _
                                     ChkNull(rec!TUR_DRSOLICITA) & Chr(9) & rec!VEN_CODIGO & Chr(9) & rec!CLI_CODIGO & Chr(9) & rec!TUR_ASISTIO & Chr(9) & ChkNull(rec!CLI_NRODOC) & Chr(9) & ChkNull(rec!TUR_DESDE) & Chr(9) & rec!TUR_TIENEMUTUAL & Chr(9) & Format(Chk0(rec!TUR_IMPORTE), "#,##0.00") & Chr(9) & ChkNull(rec!TUR_ORDEN) & Chr(9) & impreso
...
Private Function configurogrilla()
    ...
    Dim cont As Integer
    grdGrilla.FormatString = "^Horas|<Paciente|<Edad|<Telefono|<Celular|<Obra Social|<Motivo|Dr Solicitante|>Doctor|>Cod Pac|>Asistio|DNI|TUR_DESDE|TieneMutual|Importe|Orden|Impreso"
    ...
    If mNomUser = "DIGOR" Then 'ESTA CONFIGURACION LA TOMA DEL USUARIO LOGUEADO
        ...
        grdGrilla.ColWidth(16) = 750 'IMPRESO
    Else
